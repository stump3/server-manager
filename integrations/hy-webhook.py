...
def process_event(payload: dict):
    ...
    if event == "user.created":
        ...
        # modified — при редактировании существующего пользователя
        # добавляем в Hysteria если ещё не было (покрывает пользователей созданных до webhook)
        if username not in users:
            log.info(f"user.modified: {username} не найден в users.json — добавляем")
            changed = handle_user_created(username, users)
        else:
            log.debug(f"user.modified: {username} уже есть, пропускаем")
    elif event == "user.expired":
        # expired — подписка истекла, отключаем доступ
        changed = handle_user_disabled(username, users)
    else:
