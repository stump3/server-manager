...
# lib/panel/{core,cert,install,compose,mgmt_script,api,nginx/config,caddy/config,node/compose,node/api,node/install,management,warp,subpage,template,migrate,menu}.sh
#
...
for _panel_module in core cert install compose mgmt_script api nginx/config caddy/config node/compose node/api node/install management warp subpage template migrate menu; do
