...
volumes:
  ...
  valkey-socket:
    name: valkey-socket
EOFYML
    elif [ "$WEB_SERVER" = "1" ] && [ "$MODE" = "F" ]; then
        # Variant F (docs/ARCHITECTURE.md §6.1): identical to the
        # WEB_SERVER=1/MODE=1 branch above except ONE line — nginx's
        # config mount target. Everything else (remnanode's
        # network_mode: host, NET_ADMIN, /dev/shm mount for the
        # unchanged selfsteal fallback socket; Panel/DB/Redis/sub-page
        # topology) is identical, because Variant F only changes how
        # public traffic reaches Xray, not what Xray or the rest of the
        # stack does once it has it.
        cat > /opt/remnawave/docker-compose.yml << EOFYML
services:
  remnawave-db:
    image: postgres:18.3
