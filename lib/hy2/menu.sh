...
hysteria_migrate() {
    ...
    RUN echo ok >/dev/null 2>&1 || { die "Не удалось подключиться к ${rip}:${rport}"; }
...
ufw allow ${BASH_REMATCH[1]}/tcp >/dev/null 2>&1 || true"
    ...
    RUN "curl -fsSL --max-time 30 https://get.hy2.sh/ -o /tmp/hy2-install.sh && bash /tmp/hy2-install.sh; rm -f /tmp/hy2-install.sh" || { die "Ошибка установки"; }
