...
### hysteria.sh — новые пункты меню управления

- **`4) 📶 Bandwidth`** — управление пропускной способностью прямо из меню:
  - Показывает текущие значения `up`/`down` в шапке
  - Включить / изменить значения (форматы: `100 mbps`, `1 gbps`)
  - Выключить (удалить блок из `config.yaml`)
  - Предлагает перезапуск Hysteria2 после изменений

- **`5) 🎭 Маскировка`** — управление `masquerade` блоком:
  - Показывает текущий тип и URL
  - Выбор из Bing / Yahoo / Apple CDN / Hetzner / свой URL
  - Выключить маскировку
  - Предлагает перезапуск после изменений

### Примечание

Оба блока (`bandwidth` и `masquerade`) отсутствуют в конфигах установленных старой версией скрипта. Добавить через новые пункты меню без переустановки.
...
- **`user.modified`** — новое событие. При редактировании пользователя в панели: если его нет в `users.json` — добавляет автоматически. Решает проблему пользователей созданных до установки webhook — они появятся при первом же сохранении в панели
- **`user.expired`** — новое событие. При истечении подписки удаляет пользователя из `users.json` и `config.yaml`. Раньше событие игнорировалось, пользователь продолжал подключаться после истечения срока

### panel_update_script

- **Обновление `integrations/`** — при обновлении скрипта теперь также обновляются `integrations/hy-webhook.py` и `integrations/hy-sub-install.sh`. Раньше обновлялись только `lib/*.sh`, а `hy-webhook.py` на сервере оставался старым. Причина инцидента с потерей ссылок в подписках
- **Автоперезапуск hy-webhook** — если `/opt/hy-webhook/hy-webhook.py` существует на сервере, он обновляется и сервис перезапускается автоматически

### bandwidth/quic в конфиге Hysteria2

- **`bandwidth`** добавлен в шаблон конфига при установке — без него `tx: 0` при подключении клиентов, QUIC congestion control работает субоптимально, частые переподключения
- **`quic.initStreamReceiveWindow` / `maxConnReceiveWindow`** — оптимальные значения для высокой пропускной способности
