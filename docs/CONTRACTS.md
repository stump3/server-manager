...
>
> **Current status**: re-verified against `beta` HEAD `f029a71` (four
> docs-only commits on top of the baseline above; `git diff --stat` from
> baseline to this HEAD touches only `docs/`, confirmed — the code
> itself has not moved). Each contract below carries its own **Current
> status** line, one of:
...
| **Current status** | Still present — re-verified at HEAD `f029a71` (`ok()`/`info()` re-read directly, both still `echo -e` to stdout) |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`, lines unchanged |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`; citation corrected this round (call-site line numbers, explicit (a)-violation location) |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`: `grep -rn "trap.*RETURN" lib/` returns exactly those 2 lines; `grep -rn "trap.*(INT\|TERM)" lib/` returns 0 |
...
| **Current status** | Still present — re-verified at HEAD `f029a71` |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`, line unchanged |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`, all 5 files confirmed (note: `python3 - <<` counts as this pattern too, e.g. `lib/hy2/integration.sh:20,314,404,438` and `lib/panel/warp.sh:30,42,67,106` — a naive `grep "python3 <<"` undercounts to 3 files; the broader pattern confirms the original 5) |
...
| **Current status** | Still N/A — re-verified at HEAD `f029a71`: no `lib/*/py/` directory exists anywhere (`find lib -type d -name py` returns nothing), `integrations/` holds only `hy-sub-install.sh` and `hy-webhook.py` (the latter pre-dates this contract and is not yet retrofitted) |
...
| **Current status** | Still present — re-verified at HEAD `f029a71`, same 2 `RETURN` traps, same 0 `INT`/`TERM` traps |
