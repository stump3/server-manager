...
| **Note (variant-f-j only)** | The specific violation described above — the caller testing `[ -z "$_reg_out" ]` instead of the exit code — is resolved on the current `variant-f-j` baseline: `lib/panel/node/install.sh` now checks the propagated exit code directly (`if ! _reg_out=$(panel_node_register ...); then`), and `panel_node_register()` (`lib/panel/node/api.sh`) no longer writes UI text to stdout on any path. This note is scoped to `variant-f-j` only; the `beta @ 3313560` baseline, violation record, and verdict above are unchanged and still describe `beta`. This document remains a `beta`-baseline contract record, not a canonical `variant-f-j` contract source — see `docs/CORE_RUNTIME_CONTRACTS.md` for that branch's status |
...
| **Note (variant-f-j only)** | The specific violations described above are resolved on the current `variant-f-j` baseline: `lib/ui/output.sh` now routes `ok()`/`info()`/`warn()`/`err()`/`step()`/`detail()` to stderr (closing the (a) path for both functions); `panel_node_register()` (`lib/panel/node/api.sh`) writes to stdout only once, on the final success line; `panel_get_token()` (`lib/panel/core.sh`) signals failure via `warn(); return 1` with no internal `err()`/`die()` call (closing the (c) path). This note is scoped to `variant-f-j` only; the `beta @ 3313560` baseline, violation record, and verdict above are unchanged and still describe `beta`. This document remains a `beta`-baseline contract record, not a canonical `variant-f-j` contract source — see `docs/CORE_RUNTIME_CONTRACTS.md` for that branch's status |
